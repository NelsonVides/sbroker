...
-spec open_time(State) -> Open | closed when
    State :: state(),
    Open :: integer().
open_time(#state{map = Map, min = Min, small_time = Small, open_next = Next}) when
    map_size(Map) < Min
->
    min(Small, Next);
open_time(#state{map = Map, max = Max, open_next = Next}) when
    map_size(Map) < Max, is_integer(Next)
->
    Next;
open_time(#state{}) ->
    closed.
