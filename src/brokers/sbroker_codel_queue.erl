...
-spec handle_timeout(Time, State) -> {State, NextTimeout} when
    ...
    State :: state(),
    NextTimeout :: integer() | infinity.
handle_timeout(_, #state{len = Len, min = Min} = State) when
    Len =< Min
->
    {State, infinity};
handle_timeout(Time, #state{timeout_next = TimeoutNext} = State) when
    TimeoutNext > Time
->
    {State, TimeoutNext};
handle_timeout(Time, #state{drop_first = dropping, drop_next = DropNext} = State) when
    DropNext > Time
->
    {State, DropNext};
handle_timeout(Time, #state{drop_first = DropFirst} = State) when
    is_integer(DropFirst) andalso DropFirst > Time
->
    {State, DropFirst};
...
in(Len, Q, Time, #state{timeout_next = TimeoutNext, min = Min} = State) when
    TimeoutNext > Time, Len > Min
->
...
out(
    ...
    Len,
    Time,
    #state{timeout_next = TimeoutNext} = State
...
out(
    ...
    case queue:out(Q) of
        ...
        {{value, {SendTime, From, Value, Ref}}, NQ} when
...
out_r(
    {{value, {SendTime, From, Value, Ref} = Item}, Q},
    Len,
    Time,
    #state{min = Min} = State
) ->
...
out_r_empty(Item, Time, #state{drop_first = DropFirst} = State) when
    DropFirst =< Time
->
    drop_item(Time, Item),
...
timeout_next(#state{len = Len, min = Min, timeout_next = TimeoutNext}) when
    Len > Min
->
    TimeoutNext;
timeout_next(_) ->
...
drop_next(Min, Len, Q, Time, #state{drop_next = DropNext} = State) when
    DropNext > Time, Len > Min
->
    {State#state{len = Len, queue = Q}, DropNext};
drop_next(Min, Len, Q, Time, #state{count = C, drop_next = DropNext} = State) when
    Len > Min
->
    {{value, Item}, NQ} = queue:out(Q),
    drop_item(Time, Item),
...
drop_control(
    ...
    C > 2 andalso Time - DropNext < 8 * Interval
->
...
change(Time, #state{drop_first = DropFirst, interval = Interval} = State) when
    is_integer(DropFirst) andalso DropFirst > Time + Interval
->
...
change(
    ...
    is_integer(DropNext) andalso DropNext > Time + Interval
->
