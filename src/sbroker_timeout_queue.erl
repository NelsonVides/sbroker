...
-spec handle_timeout(Time, State) -> {State, TimeoutNext} when
    ...
    State :: #state{},
    TimeoutNext :: integer() | infinity.
handle_timeout(Time, #state{timeout_next = TimeoutNext, len = Len, min = Min} = State) when
    TimeoutNext > Time, Len > Min
->
    {State, TimeoutNext};
handle_timeout(Time, #state{len = Len, min = Min, queue = Q} = State) when
    Len > Min
->
...
first_timeout_next(_, infinity) ->
    infinity;
first_timeout_next(Time, Timeout) when
    is_integer(Timeout) andalso Timeout >= 0
->
    Time.

in(TimeoutNext, Len, Q, Time, #state{min = Min} = State) when
    TimeoutNext > Time, Len > Min
->
...
out({{value, {SendTime, From, Value, Ref}}, Q}, Len, _, #state{min = Min} = State) when
    Len < Min
->
...
out_r(
    ...
    Len,
    Time,
    #state{timeout = Timeout, min = Min} = State
) when SendTime + Timeout > Time ->
