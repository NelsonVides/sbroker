...
init_it(Starter, Parent, Name, Mod, Args, Opts) ->
    ...
    try Mod:init(Args) of
        {ok, {{AskMod, AskArgs}, {BidMod, BidArgs}, MeterArgs}} when
            is_list(MeterArgs)
        ->
            Config = #config{
                mod = Mod,
...
system_code_change(
    {change, Change, [_, _, _, _, #config{mod = Mod} = Config] = Misc},
    Mod,
    _,
    _
) ->
...
system_replace_state(
    ...
    MetersRes = [
        {MeterMod, meter, _} = Replace({MeterMod, meter, Meter})
...
format_status(
    ...
    Handlers2 = [
        {Mod, Id, format_status(Mod, Opt, PDict, Handler)}
...
update_meter(
    Now,
    asking,
    #time{send = Send, empty = Empty} = Time,
    Asks,
    Bids,
    ...
    try AskMod:send_time(Asks) of
        SendTime when
            is_integer(SendTime), SendTime =< Send, SendTime >= Empty
        ->
...
update_meter(
    ...
    RelativeTime,
    Empty,
    #time{now = Prev, send = Send, seq = Seq, meters = Meters} = Time,
    Asks,
    Bids,
    Config
) ->
    ...
    case
        sbroker_handlers:meters_update(
            QueueDelay,
            ProcessDelay,
            RelativeTime,
            Now,
...
meter_stop(
    ...
    Callbacks = [
        {sbroker_queue, AskMod, stop, Asks},
...
asking(
    ...
    case handle_out(AskMod, Now, Asks) of
        ...
        {exception, Class, Reason, Stack, NAsks} ->
            asking_exception(Class, Reason, Stack, Time, NAsks, Bids, Config)
    end;
...
common(
    ...
    Bids,
    _,
    #config{mod = Mod, ask_mod = AskMod, bid_mod = BidMod} = Config
) ->
...
handle_out(Mod, Now, Queue) ->
    try Mod:handle_out(Now, Queue) of
        ...
        Class:Reason:Stack ->
            {exception, Class, Reason, Stack, Queue}
    end.
...
info_meter(
    Msg,
    State,
    #time{now = Now, meters = Meters} = Time,
    Asks,
    AskNext,
    Bids,
    BidNext,
...
asking_return(
    ...
    Callbacks = [
        {sbroker_queue, AskMod, Reason, Asks},
        {sbroker_queue, BidMod, stop, Bids}
    ],
...
asking_exception(
    ...
    Callbacks = [
        {sbroker_queue, AskMod, Reason2, Asks},
        {sbroker_queue, BidMod, stop, Bids}
    ],
...
bidding(
    ...
    case handle_out(BidMod, Now, Bids) of
        ...
        {exception, Class, Reason, Stack, NBids} ->
            bidding_exception(Class, Reason, Stack, Time, Asks, NBids, Config)
    end;
...
code_change(
    [
        State,
        #time{now = Now, send = Send, meters = Meters} = Time,
        Asks,
        Bids,
        #config{ask_mod = AskMod, bid_mod = BidMod} = Config
    ],
    ...
    Callbacks = [
        {sbroker_queue, AskMod, Asks, infinity},
        {sbroker_queue, BidMod, Bids, infinity}
    ],
    NCallbacks = sbroker_handlers:code_change(
        Send,
        Now,
        Callbacks,
        Meters,
...
change(
    State,
    {NAskMod, AskArgs, NBidMod, BidArgs, MeterArgs},
    #time{now = Now, send = Send, meters = Meters} = Time,
    Asks,
    Bids,
    #config{ask_mod = AskMod, bid_mod = BidMod} = Config
    ...
    Inits = [
        {sbroker_queue, AskMod, Asks, NAskMod, AskArgs},
    ...
    of
        {ok, [{_, _, NAsks, AskNext}, {_, _, NBids, BidNext}], {NMeters, MeterNext}} ->
            NTime = Time#time{meters = NMeters, next = MeterNext},
...
terminate(Reason, #time{meters = Meters}, Callbacks, Config) ->
    Name = report_name(Config),
    {stop, NReason} = sbroker_handlers:terminate(
        Reason,
        Callbacks,
        Meters,
        Name
