...
command(#state{mod = Mod} = State) ->
    frequency([
        {16, {call, ?MODULE, handle_update, handle_update_args(State)}},
...
init_or_change_post(
    ...
    [_, _, Manager, _, Args, Time],
    {ok, _, Timeout}
) ->
    {_, Timeout2} = Manager:init(Time, Args),
    timeout_post(Timeout2, Timeout);
init_or_change_post(
    #state{manager = Manager, manager_state = ManState},
    [Mod, _, Manager, Mod, Args, Time],
    {ok, _, Timeout}
) ->
...
handle_update_args(#state{mod = Mod, time = Time, meter = M}) ->
    ?LET(
        {QueueTime, ProcessTime, RelTime},
        {choose(0, 5), choose(0, 5), choose(-5, 5)},
        begin
            NQueueTime = erlang:convert_time_unit(
                QueueTime,
                millisecond,
                native
            ),
            NProcessTime = erlang:convert_time_unit(
                ProcessTime,
                millisecond,
                native
            ),
            NRelTime = erlang:convert_time_unit(
                RelTime,
                millisecond,
                native
            ),
...
handle_update_next(
    ...
    {NManState, Timeout} = Manager:update_next(
        ManState,
        Time,
        MsgQLen,
        QueueDelay,
...
handle_update_post(
    ...
    {Result, Timeout2} = Manager:update_post(
        ManState,
        Time,
        MsgQLen,
        QueueDelay,
