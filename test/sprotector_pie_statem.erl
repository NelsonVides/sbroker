...
args() ->
    ?LET(
        {AskTarget, AskBurst, BidTarget, BidBurst, Update, Min, Max},
        gen_args(),
        #{
...
drop_post(ExpDrop, Queue) ->
    case sprotector_server:lookup(self(), Queue) of
        ...
        ObsDrop ->
            ct:pal(
                "Drop ~p~nExpected: ~p~nObserved: ~p",
