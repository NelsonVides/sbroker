...
do_update(_, #state{first_above_time = FirstAbove, now = Now} = State) when
    Now >= FirstAbove
->
    {open, State};
do_update(_, State) ->
...
update_opening(open, #state{now = Now, open_next = OpenNext} = State) when
    Now >= OpenNext
->
    {open, State};
...
handle(#state{first_above_time = undefined} = State) ->
    {closed, State};
handle(#state{opening = false, now = Now, first_above_time = FirstAbove} = State) when
    Now >= FirstAbove
->
    {open, State};
handle(#state{updated = false} = State) ->
    {closed, State};
handle(#state{opening = true, now = Now, open_next = OpenNext} = State) when
    Now >= OpenNext
->
    {open, State};
handle(State) ->
